...
namespace LunchGame01 {
	...
	internal class RuleTileEditor : Editor {
	    ...
	    private const float k_DefaultElementHeight = 48f;
	    private const float k_PaddingBetweenRules = 13f;
	    private const float k_SingleLineHeight = 16f;
	    private const float k_LabelWidth = 53f;
		...
		private void OnDrawElement(Rect rect, int index, bool isactive, bool isfocused) {
			...
			Rect inspectorRect = new Rect(rect.xMin, yPos, rect.width - k_DefaultElementHeight * 2f - 20f, height);
			Rect matrixRect = new Rect(rect.xMax - k_DefaultElementHeight * 2f - 10f, yPos, k_DefaultElementHeight, k_DefaultElementHeight);
		...
		private class MenuItemData {
			public readonly RuleTile.TilingRule m_Rule;
			public readonly RuleTile.TilingRule.Transform m_NewValue;
		...
		private static void SpriteOnGUI(Rect rect, RuleTile.TilingRule tilingRule) {
			tilingRule.m_Sprites[0] = EditorGUI.ObjectField(new Rect(rect.xMax - rect.height, rect.yMin, rect.height, rect.height),
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule) {
			...
			case RuleTile.TilingRule.OutputSprite.Animation:
			    ...
			    tilingRule.m_AnimationSpeed = EditorGUI.FloatField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight),
			                                                       tilingRule.m_AnimationSpeed);
			    y += k_SingleLineHeight;
			    break;
			case RuleTile.TilingRule.OutputSprite.Random:
			    GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Noise");
			    tilingRule.m_PerlinScale = EditorGUI.Slider(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight),
			                                                tilingRule.m_PerlinScale, 0.001f, 0.999f);
			    y += k_SingleLineHeight;

			    GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Shuffle");
			    tilingRule.m_RandomTransform = (RuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight),
		    ...
		    if (tilingRule.m_Output != RuleTile.TilingRule.OutputSprite.Single) {
				...
				EditorGUI.BeginChangeCheck();
				int newLength = EditorGUI.DelayedIntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight),
				...
				for (int i = 0; i < tilingRule.m_Sprites.Length; i++) {
					tilingRule.m_Sprites[i] = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight),
