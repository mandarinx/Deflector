...
namespace LunchGame01 {
	...
	public class RuleTile : TileBase {
		...
		public class TilingRule {
			public readonly Neighbor[]        m_Neighbors;
			...
			public TilingRule(Transform mRuleTransform, Transform mRandomTransform) {
			...
			foreach (TilingRule rule in m_TilingRules) {
			    ...
			    switch (rule.m_Output) {
                    ...
                    case TilingRule.OutputSprite.Random:
                        int r = Mathf.FloorToInt(GetPerlinValue(position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length);
                        int index = Mathf.Clamp(r, 0, rule.m_Sprites.Length - 1);
			...
			for (int y = -1; y <= 1; y++) {
				for (int x = -1; x <= 1; x++) {
					if (x != 0 || y != 0) {
						...
						if (rule.m_Neighbors[index] == TilingRule.Neighbor.This &&
						    tile != this ||
						    rule.m_Neighbors[index] == TilingRule.Neighbor.NotThis &&
						    tile == this) {
							return false;
						}
					}
		...
		private static int GetIndexOfOffset(Vector3Int offset) {
		...
		public static Vector3Int GetRotatedPos(Vector3Int original, int rotation) {
			switch (rotation) {
				case 0:
					return original;
				case 90:
		...
		public static Vector3Int GetMirroredPos(Vector3Int original, bool mirrorX, bool mirrorY) {
			return new Vector3Int(original.x * (mirrorX ? -1 : 1),
			                      original.y * (mirrorY ? -1 : 1),
			                      original.z);
		}
	}
}
