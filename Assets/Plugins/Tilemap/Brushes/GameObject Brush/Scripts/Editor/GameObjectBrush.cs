...
namespace UnityEditor {
	...
	[CustomGridBrush(hideAssetInstances:  true, 
					 hideDefaultInstance: false, 
					 defaultBrush:        false, 
					 defaultName:         "GameObject Brush")]
	public class GameObjectBrush : GridBrushBase {
		...
		public int cellCount => m_Cells?.Length ?? 0;
		...
		public void Init(Vector3Int initSize) {
			Init(initSize, Vector3Int.zero);
			SizeUpdated();
		}

		public void Init(Vector3Int initSize, Vector3Int initPivot) {
			m_Size = initSize;
			m_Pivot = initPivot;
			SizeUpdated();
		}
		...
		public override void Rotate(RotationDirection direction, Grid.CellLayout layout) {
			...
			Matrix4x4 rotation = Matrix4x4.TRS(
				Vector3.zero, 
				Quaternion.Euler(0f, 0f, direction == RotationDirection.Clockwise ? 90f : -90f), 
				Vector3.one);
		...
		public override void Pick(GridLayout gridLayout, 
			...
			Reset();
			UpdateSizeAndPivot(
				new Vector3Int(position.size.x, position.size.y, 1), 
		...
		private void PickCell(Vector3Int position, Vector3Int brushPosition, GridLayout grid, Transform parent) {
			...
			Vector3 cellCenter = grid.LocalToWorld(grid.CellToLocalInterpolated(position + Vector3.one * .5f));
...
//				newInstance.hideFlags = HideFlags.HideAndDontSave;
		...
		private bool ValidateCellPosition(Vector3Int position) {
			...
			if (!valid) {
				throw new ArgumentException($"Position {position} is an invalid cell position. "+
											$"Valid range is between [{Vector3Int.zero}, {size}).");
			}
			return valid;
		}
		...
		private static void SetSceneCell(GridLayout grid, 
										 Transform parent, 
										 Vector3Int position, 
										 GameObject go, 
										 Vector3 offset, 
			...
			Vector3 local = new Vector3Int(position.x, position.y, position.z) + Vector3.one * .5f;
			instance.transform.position = grid.LocalToWorld(grid.CellToLocalInterpolated(local));
	...
	public class GameObjectBrushEditor : GridBrushEditorBase {
		...
		public override void OnPaintSceneGUI(GridLayout 		gridLayout, 
											 ...
											 bool 				executing) {
			BoundsInt gizmoRect = 
				   tool == GridBrushBase.Tool.Paint
				|| tool == GridBrushBase.Tool.Erase
				? new BoundsInt(position.min - brush.pivot, brush.size)
				: position;
